(ns ak-gen.core
  ...
  (:require [clj-path.core :as path]
            ...
            [ak-gen.template :as template]))

(defn modkey->str [modkey]
  (str "<" (name modkey) ">"))

(defn ->script [{:as hotkey-info :keys [modifiers keys]}]
  (str "keyboard.send_keys(\""
       (when-not (empty? modifiers)
         (str (str/join "+" (map modkey->str modifiers)) "+"))
       keys
       "\")"))
...
(defn ->hotkey-settings [{:as hotkey-info :keys [modifiers hotkey]}]
  {:description    (str (str/join "_" (map name modifiers)) "_" hotkey)
   :store          {}
   :showInTrayMenu false
   :type           "script"
   :modes          [3]
   :usageCount     0
   :hotkey         {:modifiers (map modkey->str modifiers) :hotKey hotkey}
   :prompt         false
...
(defn write-hotkey-file [{:as hotkey-info :keys [modifiers hotkey keys blind? dirpath]}]
  (if blind?
    (doseq [mods (combo/subsets [:ctrl :shift :alt])]
      (write-hotkey-file (-> hotkey-info
                             (assoc ,, :blind? false)
                             (update ,, :modifiers #(vec (set (concat mods %)))))))
    (let [basename (str (when (not-empty modifiers)
                          (str (str/join "_" (map name modifiers)) "_"))
                        hotkey)]
...
(comment
  (write-hotkey-files
   [{:modifiers [:hyper]
     ...
     :keys "<up>"
     :blind? true}])
  (-main "/home/shimotoku/tmp/ak-gen-input.csv"))
